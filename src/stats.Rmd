...
latex_table <- all_results %>%
  ...
  kable(
    ...
    booktabs = TRUE, 
    digits = 4,
    col.names = c("", "Term", "Estimate", "Std. Error", "t-value", "Corrected p-value", "Significance"),
    align = "lcccccc", 
    escape = FALSE
  ) %>%
  kable_styling(
...
metric_rows <- all_results %>%
  ...
  summarize(
    count = n(),
    formatted_name = first(formatted_metric)
  ) %>%
  ungroup()
...
# display the table
cat(substr(latex_table, 1, 500))  # shows the first 500 characters of the string
