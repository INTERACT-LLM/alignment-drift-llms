...
textdescriptives_df <- read_csv(here("metrics", paste0("v", version, "_textdescriptives.csv"))) %>%
  filter(role == role_to_keep) %>%
  rename(message_length = doc_length)
...
fit_lmer_models <- function(data, models, metrics, output_dir) {
  ...
  for (model_name in models) {
    for (metric in metrics) {
      ...
      formula <- as.formula(paste(metric, "~ group + (1 | id)"))
...
metrics <- c("message_length", "dependency_distance_mean")
...
html_table <- all_results %>%
  ...
  kable(
    format = "html", booktabs = TRUE, digits = 4, 
    col.names = c("Model", "Metric", "Term", "Estimate", "Std. Error", "t-value", "Corrected p-value", "Significance"), 
    align = "lccccccc", escape = FALSE
  ) %>%
...
```{r}
latex_table %>% save_kable(file = file.path(output_dir, "results_table.tex"), self_contained = T)
```
